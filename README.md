...
> - Retriever API 추가 (Standard / kNN / RRF)
> - Retriever 클라이언트 사이드 실행 기본 활성화 — Basic 라이선스에서도 RRF 사용 가능 (`useClientSideRetriever`)
...
> **⚠️ 실행 모드**
> 본 라이브러리는 v2.0 부터 **클라이언트 사이드 실행** 을 기본으로 제공하여, RRF 등 일부 retriever 를 Basic 라이선스에서도 사용할 수 있습니다.
> - **Basic 라이선스 (기본)**: Standard / kNN 은 ES 네이티브로, RRF 는 클라이언트 사이드 머지로 실행됩니다.
> - **네이티브 ES 경로로 전환**: `request.useClientSideRetriever = false` 로 ES 네이티브 retriever 경로를 사용. 이 경우 RRF, Linear, Rule, Text Similarity Re-ranker 등은 Enterprise 라이선스가 필요합니다.
> - 동작 흐름과 한계는 아래 [클라이언트 사이드 실행 (기본값)](#클라이언트-사이드-실행-기본값) 섹션 참조.

Retriever 는 Elasticsearch 8.14+ 에서 도입된 통합 검색 인터페이스로, BM25, kNN, 하이브리드 검색을 쉽게 구현할 수 있습니다.
...
> **✅ Basic 라이선스 지원**: 클라이언트 사이드 실행(기본값)으로 RRF 를 Basic 라이선스에서도 사용할 수 있습니다.
> 네이티브 ES retriever 경로(`useClientSideRetriever = false`)로 전환 시에는 Enterprise 라이선스가 필요합니다.

Reciprocal Rank Fusion 을 사용하여 여러 검색 전략을 결합합니다.
...
> **참고**: Retriever 를 사용할 때는 `setQueryDsl()` 과 함께 사용할 수 없습니다. Retriever 가 우선합니다.

### 클라이언트 사이드 실행 (기본값)

v2.0 부터 retriever 는 기본적으로 **클라이언트 사이드** 에서 분해/머지 실행되어, ES Basic 라이선스에서도 RRF 등 고급 retriever 를 사용할 수 있습니다. 빌더 API 는 그대로 유지되며 동작 모드만 플래그로 전환됩니다.
...
flowchart LR
    ...
    B -- No --> C[esClient.search 호출 / 네이티브]
    B -- Yes --> D[EsRetrieverPipeline]
    D --> E[자식 retriever 실행 + RRF 머지]
    E --> F[ResultAssembler: source/highlight 보강]
    F --> G[BtEsSearchResponse 합성]
```
...
request.setRetriever(rrfRetriever);

// (기본) 클라이언트 사이드 실행 — Basic 라이선스 OK
request.useClientSideRetriever = true;

// 네이티브 ES retriever 경로 — RRF 등은 Enterprise 라이선스 필요
request.useClientSideRetriever = false;
```

#### 실행 시맨틱 요약

| Retriever | 클라이언트 사이드 동작 | 추가 round-trip |
|---|---|---|
| Standard | ES `search` 1회 (query/filter inline) | 0 |
| kNN | top-level `knn` 으로 변환 후 `search` 1회 | 0 |
| RRF | 자식 N개 병렬 dispatch → RRF 머지 → 슬라이스 → (옵션) 보강 | N (자식) + 0~2 (union·aggs·highlight) |

#### 페이징

- 모든 retriever 는 **stateless re-execution** (페이지마다 전체 재실행) 방식입니다.
- **페이징 제약**:
  - RRF: `rank_window_size >= from + size` 충족 필요. 미만이면 명시적 에러.
  - kNN: `k >= from + size` 충족 필요. 미만이면 명시적 에러.
  - Standard: 제약 없음 (ES 가 처리).
- 깊은 페이징은 `rank_window_size` / `k` 를 함께 키워야 하며 자식별 fetch 비용이 선형 증가합니다. 실무에서는 상위 20–50 결과 정도가 retriever 의 의미 있는 사용 범위입니다.

#### `totalCount` (track_total_hits) 시맨틱

`request.trackTotalHits` 값에 따라 RRF total 계산 방식이 달라집니다 (Hybrid):

| trackTotalHits | 자식 구성 | total 계산 | relation |
|---|---|---|---|
| `true` (기본) | 모든 자식 standard | union 쿼리 (`bool.should + minimum_should_match:1`) 1회 추가 발사로 exact | `eq` |
| `true` | knn / nested rrf 자식 포함 | `Σ child.total` (best-effort, 중복 가능) | `gte` |
| `false` / `0` | — | merged unique candidate 수 | `eq` |
| 정수 N | — | 위 경로 후 `min(value, N)` cap | (각 경로에 따름) |

> ES native RRF 의 `hits.total` 은 자식 union 의 unique 문서 수에 가깝습니다. 본 라이브러리의 클라이언트 사이드 RRF 도 가능한 범위에서 같은 시맨틱을 따르되, kNN 자식이 포함된 경우 정확한 union 계산이 어려워 `gte` 근사로 폴백합니다.

#### Aggregations (집계)

| Retriever | 집계 스코프 | 추가 round-trip |
|---|---|---|
| Standard | ES 가 처리 (전체 매치 셋) | 0 |
| kNN | ES 가 처리 (top-k) | 0 |
| RRF | **머지된 candidate pool ID 에 한정** (`{ query: { ids: { values: [...] } }, size:0, aggregations }`) | +1 |

> RRF 의 집계 스코프는 candidate pool (≤ Σ rank_window_size) 입니다. ES native union 스코프보다 작을 수 있으니, 정확한 union 집계가 필요하면 `useClientSideRetriever = false` 로 네이티브 경로를 사용하세요.

#### 거부되는 조합

다음 조합은 클라이언트 사이드 실행에서 명시적 에러를 발생시킵니다:

- RRF + `sort` — RRF 점수가 곧 정렬 키이므로 의미 없음.
- RRF + `searchAfter` — stateless re-execution 과 호환되지 않음.
- RRF + `rank_window_size < from + size`.
- kNN + `k < from + size`.

#### 한계 (V1)

- 자식 결과 캐싱 없음 — 페이지마다 자식 재실행.
- msearch 번들 최적화 없음 — 자식별 개별 round-trip (V2 예정).
- 중첩 RRF 의 union total 은 모든 leaf 가 standard 인 경우에만 exact.

자세한 설계 결정 사항은 [`docs/retriever-strategy.md`](docs/retriever-strategy.md) 를 참조하세요.
